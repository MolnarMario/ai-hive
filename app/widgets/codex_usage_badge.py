"""Top-bar ChatGPT/Codex five-hour usage readout."""
...
class CodexUsageBadge(UsagePillBadge):
    """The signed-in ChatGPT account's Codex primary usage window."""
    ...
    def _loading_text(self) -> str:
        return "ChatGPT/Codex 5h usage, reading..."

    def _format_limit(self) -> str:
        return codex_usage.format_limit(self._limit)

    def _unreadable_text(self) -> str:
        return "ChatGPT/Codex usage unreadable, click to refresh"

    def _build_tooltip(self) -> str:
        if self._loading:
            return "Reading ChatGPT/Codex 5-hour usage..."
        if self._limit is None and self._unreadable:
            return ("ChatGPT/Codex usage could not be read.\n"
                    f"Last attempt failed: {self._unreadable}\nClick to refresh.")
        if self._usage is None or self._limit is None:
            return "ChatGPT/Codex 5-hour usage"
        lines = ["ChatGPT/Codex" + (f" ({self._usage.plan})" if self._usage.plan else ""),
